optionPricer = function(Sym, RF, PriceRange)
{
        ...
        for (i in 1:nrow(puts)) {
                ...
                puts[i, 26] = "p"
                puts[i, 27] = 1-pnorm(log(puts$BreakEvenStockPrice[i]/puts$U_LastTraded[i]),(puts$U_MeanDailyReturn_1year[i]-1)*puts$O_RemainingDays[i], (puts$U_Vol1year[i]/(252^.5))*(puts$O_RemainingDays[i]^.5))
                puts[i, 28] = 1-pnorm(log(puts$Strike[i]/puts$U_LastTraded[i]),(puts$U_MeanDailyReturn_1year[i]-1)*puts$O_RemainingDays[i], (puts$U_Vol1year[i]/(252^.5))*(puts$O_RemainingDays[i]^.5))
        }
